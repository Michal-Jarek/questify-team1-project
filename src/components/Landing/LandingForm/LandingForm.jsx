...
import { Wrapper, TextFieldStyled, RegistrationLInk, FormButton, Paragraph } from './LandingForm.styled';
	...
	const { handleSubmit, values, handleChange, touched, errors, resetForm } = useFormik({
		...
		onSubmit: async values => {
			await login(values)
				...
				.catch(() => {
					Notiflix.Notify.failure('Please check your email address or password');
				});
	...
	return (
		<Wrapper>
			<form onSubmit={handleSubmit}>
				<Paragraph>
					Choose your name to <RegistrationLInk onClick={() => setIsOpen(true)}>sign up</RegistrationLInk> or
					log in
				</Paragraph>
				<TextFieldStyled
					fullWidth
